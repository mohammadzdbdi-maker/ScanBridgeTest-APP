...
public sealed class LocalizationManager
{
    ...
    ["HighUsageBarcodePanel"] = ("High-usage barcodes", "بارکد پرمصرف"),
    ["PriceLookupPanel"] = ("Price inquiry", "استعلام قیمت"),
    ["HighUsageWidgetEnable"] = ("📦 Enable high-usage barcode bank", "📦 فعال‌سازی بانک بارکد پرمصرف"),
    ...
    ["AppUpdateSectionTitle"] = ("🔄 Update", "🔄 بروزرسانی"),
    ["CheckForUpdateButton"] = ("Check for update", "بررسی بروزرسانی"),
    ["UsbInternetSectionTitle"] = ("🔌 Internet over USB connection", "🔌 اینترنت جانبی در اتصال USB"),
    ["UsbInternetSectionDescription"] = ("When the phone is connected by cable and the system's internet has dropped, this button raises the USB adapter's metric so internet continues over Wi-Fi/Ethernet and scanning over the cable keeps working (asks for admin permission once).", "وقتی گوشی با کابل وصل است و اینترنت سیستم قطع شده، این دکمه متریک آداپتور USB را بالا می‌برد تا اینترنت از مسیر وای‌فای/اترنت ادامه پیدا کند و اسکن با کابل هم سالم بماند (یک بار اجازه‌ی مدیر می‌گیرد)."),
    ["FixUsbInternetButton"] = ("Fix USB internet conflict", "رفع تداخل اینترنت USB"),
    ...
    ["TtTeckPlusOnlyWhenAnItemRegisteredInCargoDeliveryHasANearExpirationDateItIsFlaggedInTheSamePanel"] = ("Pro plan only - when an item registered in \"Cargo delivery\" has a near expiration date, it is flagged in the same panel.", "مخصوص پلن حرفه‌ای - وقتی کالایی در «تحویل بار» با تاریخ انقضای نزدیک ثبت شود، در همان پنل هشدار داده می‌شود."),
